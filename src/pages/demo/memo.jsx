...
function DemoMemo(props) {
  ...
  const MyComponent = React.memo(props => {
    console.log("new ender");
    return (<div>implicit memoized component</div>)
  });

  const increateCount = () => {
    console.log('Tinh count');
    if(count == 2) {
      setCount(2);
    } else {
      setCount(count+1);
  ...
  return (
    <div style={{ padding: "15px" }}>
      ...
      <button
        onClick={() => {
          const next = wordIndex + 1 === words.length || wordIndex == 1 ? 1 : wordIndex + 1;
          setWordIndex(next);
        }}
      >
        Next word
      ...
      <h1><MyComponent count={count}/></h1>
      <p>Counter: {count}</p>
      <button onClick={() => increateCount()}>Increment</button>
    </div>
  );
}
